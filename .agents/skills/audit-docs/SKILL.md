...
mechanical — run them before judging content quality. Full convention (bundled with this skill):
[guidelines/traceability.md](guidelines/traceability.md) — canonical in this repo at
`docs/product/guidelines/traceability.md`.
...
| **Convention mirrors** | Every bundled mirror is byte-identical to the canonical convention — `cmp -s docs/product/guidelines/traceability.md .agents/skills/<skill>/guidelines/traceability.md` for `create-entity-lifecycle`, `create-flow-documentation`, `modify-flow`, `audit-docs` (the file carries no relative links outside its fenced examples, so equality is the requirement) | ⚠️ Stale — skill teaches outdated rules; re-sync with `cp` |
