...
| --- | --- |
| Ad-hoc Zod schemas defined inside route handlers | Schemas centralized in `@sessioflow/api-definitions`; interface tests validate against contracts |
| Duplicated schemas between `api-definitions` and module `.command.ts` DTOs | Interface/controller tests bound to the contract package; review |
| Forgetting `dist/` rebuilds after schema changes | AGENTS.md rebuild discipline (`npx turbo build --filter=@sessioflow/<package>`) |
