...
| --- | --- |
| Reaching for `inversify`, `tsyringe`, decorators or React context DI | New-dependency ask-first gate; factory-function convention in existing containers as reference |
| Module-level singletons / service-locator style wiring | `container.ts` as sole composition root per module (create-module skill scaffolds it) |
| Wiring outside the container, creating circular imports | Arch rules on `interfaces/` imports; typecheck |
