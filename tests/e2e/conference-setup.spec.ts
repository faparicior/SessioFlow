...
test.describe('Conference Setup E2E', () => {
  ...
  test('should create a conference with CfP configuration (Happy Path)', async ({page}) => {
    ...
    await expect(page.getByText('https://sessioflow.app/cfp/tech-conference-2026')).toBeVisible();

    // Step 6: Submit the form
    await page.getByRole('button', {name: /create conference/i}).click();

    // Step 7: Wait for form submission to complete
    await page.waitForLoadState('networkidle');

    // Step 8: Verify success notification
    await expect(page.getByText(/conference created successfully/i)).toBeVisible();

    // Step 9: Verify redirect to conference dashboard
    await page.waitForURL(/\/conferences\//);

    // Step 10: Verify CfP link is displayed in dashboard
    await expect(page.getByText(/cfp\/tech-conference-2026/i)).toBeVisible();
  });
  ...
  test('should reject conference with free tier limit exceeded', async ({page}) => {
    // Fill form
    await page.getByLabel('Conference Name').fill('Too Many Conferences');
    await page.getByLabel('CfP Start Date').fill('2026-08-01');
    await page.getByLabel('CfP Start Date').blur();
    await page.getByLabel('CfP End Date').fill('2026-09-30');
    await page.getByLabel('CfP End Date').blur();
  ...
  test('should reject conference with past CfP date', async ({page}) => {
    ...
    await page.getByLabel('Conference Name').fill('Past Date Conference');
