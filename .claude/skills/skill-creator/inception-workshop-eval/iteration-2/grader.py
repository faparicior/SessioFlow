#!/usr/bin/env python3
"""
Grader for Inception Workshop Skill Evaluation - Iteration 2
...
def grade_eval_001_with_skill():
    ...
    output_file = "iteration-2/eval-001-with-skill/outputs/product-vision.md"
...
def grade_eval_002_with_skill():
    ...
    output_dir = "iteration-2/eval-002-with-skill/outputs"
    ...
    # Check for consistent product name - should mention SessioFlow frequently
    mentions = all_content.lower().count("sessioflow")
    passed = mentions >= 15  # Should mention product name multiple times across all docs
    results.append({
...
def grade_eval_003_with_skill():
    ...
    output_file = "iteration-2/eval-003-with-skill/outputs/tradeoffs.md"
...
def grade_baseline_outputs():
    ...
    # Eval 001 baseline
    try:
        with open("iteration-2/eval-001-baseline/outputs/product-vision.md", 'r') as f:
            content = f.read()
...
def main():
    print("=" * 60)
    print("INCEPTION WORKSHOP SKILL EVALUATION - ITERATION 2")
    print("=" * 60)
    print()
    ...
    with open("iteration-2/grading.json", 'w') as f:
        json.dump(grading, f, indent=2)
    
    print("Grading results saved to iteration-2/grading.json")
