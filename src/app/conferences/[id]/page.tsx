 ...
 */
export default function ConferenceDetailPage({params}: {readonly params: Promise<{id: string}>}) {
        ...
        <CardContent className='space-y-4'>
          ...
          <div>
            ...
            <ul className='list-disc list-inside text-sm text-muted-foreground'>
              <li>Start Date: {new Date(conference.cfpStartDate).toLocaleDateString()}</li>
              <li>End Date: {new Date(conference.cfpEndDate).toLocaleDateString()}</li>
              <li>Status: {conference.cfpStatus}</li>
              {conference.maxSubmissions ? <li>Max Submissions: {conference.maxSubmissions}</li> : null}
              <li>Requires Approval: {conference.requiresApproval ? 'Yes' : 'No'}</li>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className='flex items-center gap-2 p-2 bg-muted rounded-md'>
              ...
              <Button
                ...
                variant='outline'
                size='sm'
                onClick={async () => navigator.clipboard.writeText(conference.cfpUrl)}
              >
                Copy
              </Button>
            </div>
          ...
          <div className='flex gap-2'>
            ...
            <Button variant='outline' onClick={() => window.open(conference.cfpUrl, '_blank')}>
              Open CfP
            </Button>
          </div>
        </CardContent>
