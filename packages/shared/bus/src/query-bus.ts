...
export class InMemoryQueryBus implements IQueryBus {
  ...
  register<TQuery extends IQuery, TResponse>(
    ...
    if (this.handlers.has(queryClass)) {
      throw new Error(
        `Query handler for ${queryClass.name} is already registered.`
      );
    }
  ...
  async dispatch<TQuery extends IQuery, TResponse>(
    query: TQuery
  ): Promise<TResponse> {
    ...
    for (let i = this.middlewares.length - 1; i >= 0; i--) {
      ...
      pipeline = () =>
        middleware.execute(query, current) as Promise<TResponse>;
    }
