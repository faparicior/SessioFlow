...
console.log(`🔍 Running Architecture Check${targetArg ? ` for target: ${targetArg}` : ' across monorepo'}...`);
      ...
      defineCondition('valueObjectConventions', (matchedClasses) => {
        return matchedClasses.map((cls) => {
          ...
          if (!/private\s+constructor\b/.test(fileSource)) {
            return { rule: 'private constructor', element: name, file: relPath, line: 0, message: `VO "${name}" missing private constructor.` };
          }
          if (!/static\s+(create|fromString)\b/.test(fileSource)) {
            return { rule: 'static factory', element: name, file: relPath, line: 0, message: `VO "${name}" missing static create/fromString factory.` };
          }
          if (!/get\s+value\s*\(\)/.test(fileSource)) {
            return { rule: 'get value()', element: name, file: relPath, line: 0, message: `VO "${name}" missing get value() getter.` };
          }
          if (!/\bequals\s*\(/.test(fileSource)) {
            return { rule: 'equals(other)', element: name, file: relPath, line: 0, message: `VO "${name}" missing equals(other) method.` };
          }
          return null;
        }).filter(Boolean);
      })
    ...
    .satisfy(
      defineCondition('domainEntityNoPrimitives', (matchedClasses) => {
        return matchedClasses.map((cls) => {
          const relPath = getElementFile(cls);
          if (targetArg && !relPath.includes(targetArg)) return null;
          if (relPath.includes('/value-objects/') || relPath.includes('/exceptions/') || relPath.includes('/events/')) return null;

          const fileSource = cls.getSourceFile().getFullText();
          const dataTypeMatch = fileSource.match(/type\s+\w+Data\s*=\s*\{([^}]+)\}/);
          if (!dataTypeMatch) return null;

          const primitiveMatches = dataTypeMatch[1].match(/(\w+)\s*:\s*(string|number|boolean)\b/g);
      ...
      defineCondition('domainEventConventions', (matchedClasses) => {
        return matchedClasses.map((cls) => {
          ...
          if (!name.endsWith('Event')) {
            return { rule: 'event name suffix', element: name, file: relPath, line: 0, message: `Domain Event "${name}" must end with "Event".` };
          }
          if (!/\btoJSON\s*\(/.test(fileSource)) {
            return { rule: 'toJSON serialization', element: name, file: relPath, line: 0, message: `Domain Event "${name}" must implement toJSON().` };
          }
          return null;
        }).filter(Boolean);
      })
      ...
      defineCondition('domainExceptionConventions', (matchedClasses) => {
        return matchedClasses.map((cls) => {
          ...
          if (!name.endsWith('Error')) {
            return { rule: 'exception name suffix', element: name, file: relPath, line: 0, message: `Domain Exception "${name}" must end with "Error".` };
          }
          if (!/extends\s+(Domain\w+Error|EntityNotFoundError)\b/.test(fileSource)) {
            return { rule: 'extends DomainError', element: name, file: relPath, line: 0, message: `Domain Exception "${name}" must extend DomainError.` };
          }
          return null;
        }).filter(Boolean);
      })
